"""MUTANT: keys canonical bytes on object identity.

Defect: appends the argument's `id()` to the serialization. Logically equal
documents built independently differ in identity, violating §4 / §6 determinism.
Should fail only test_lp03_determinism_across_env_matrix.
"""
...
EXPECTED_FAIL = ["test_lp03_determinism_across_env_matrix"]
...
def _serialize(doc: ParsedDocument) -> bytes:
    ...
    # defect: identity-dependent suffix
    chunks.extend((b"\nobject-id:", str(id(doc)).encode("ascii"), b"\n"))
