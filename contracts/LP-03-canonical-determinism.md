...
Test name is `test_lp03_<id>` — the ADR-0030 pure function `test_<LP_ID_normalized>_<id>` with LP-ID lowercased and separator removed (`LP-03` → `lp03`, per ADR-0030 A2.2, matching frozen LP-09's `test_lp09_<id>`). One test function per id (bijective); **no pytest parametrization**. A case that needs a matrix of conditions loops internally within its single test function, keeping the pytest node-id byte-stable — the id *is* the node.
...
| 0.6 | 2026-09-20 | DRAFT — reverted the v0.4 test-name change. The ADR-0030 deriver is A2.2 *lowercased, separator removed* (`LP-03`→`lp03`, `test_lp03_<id>`), matching frozen LP-09's `test_lp09_<id>` — NOT hyphen→underscore. The v0.4 `test_lp_03_` prefix was the error: it failed `validate-naming` with all 5 authored tests flagged as orphans. §7 restored to `test_lp03_<id>`. No other change; requires a verifier re-author. |
